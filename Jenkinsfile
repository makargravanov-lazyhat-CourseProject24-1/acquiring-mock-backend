    ...
    stages {
        ...
        stage("Deploy"){
            steps {
                ...
                sh "sudo docker run --detach --name ${SERVICE_NAME} -e HTTP_PORT=${HTTP_PORT} -e DB_URL=${DB_URL} -e DB_USERNAME=${DB_USERNAME} -e DB_PASSWORD=${DB_PASSWORD} --network=service_postgresql--service --network=service_nginx--acquiring-mock-backend jetlabs/${SERVICE_NAME}:latest"
            }
        }
    }
}
